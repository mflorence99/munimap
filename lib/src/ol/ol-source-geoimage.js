// 🔥 modified 4/3/2024 to optionally show a feathered edge around the image
...
var ol_source_GeoImage = class olsourceGeoImage extends ol_source_ImageCanvas {
  constructor(opt_options) {
    ...
    this.filter = opt_options.imageFilter;
    ...
    // Mask of the image
    this.feather = opt_options.imageFeather;
    this.featherFilter = opt_options.imageFeatherFilter;
  ...
  calculateImage(extent, resolution, pixelRatio, size) {
    ...
    function draw(mask, filter) {
      ...
      var dx =
        (((this._image.naturalWidth / 2 - this.crop[0]) * this.scale[0]) /
          resolution) *
        pixelRatio;
      var dy =
        (((this._image.naturalHeight / 2 - this.crop[1]) * this.scale[1]) /
          resolution) *
        pixelRatio;
    ...
    if (this.feather)
      draw.bind(this)(this.feather, this.featherFilter ?? 'none');
    draw.bind(this)(this.mask, this.filter ?? 'none');
    return canvas;
  }
  /**
  ...
  setCrop(crop) {
    ...
    if (crop) {
      switch (typeof crop) {
        case 'number':
          crop = [
            crop,
            crop,
            this._image.naturalWidth - crop,
            this._image.naturalHeight - crop
          ];
          break;
        case 'object':
      ...
      this.crop = [
        Math.max(0, crop[0]),
        Math.max(0, crop[1]),
        Math.min(this._image.naturalWidth, crop[2]),
        Math.min(this._image.naturalHeight, crop[3])
      ];
    } else
    ...
    this._imageSize = [
      this.crop[2] - this.crop[0],
      this.crop[3] - this.crop[1]
    ];
    this.changed();
  }
